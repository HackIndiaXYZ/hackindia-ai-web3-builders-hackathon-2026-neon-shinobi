...
def build_notifications(alerts_file=None, policies_file=None, alerts_data=None, policies_data=None) -> List[Dict[str, Any]]:
    """
    alerts_data / policies_data: pass these directly to run the agent on
    in-memory data (used by the live backend loop). If omitted, falls back
    to reading from disk (used by the CLI / standalone run below).
    """
    if alerts_data is not None:
        raw_alerts = alerts_data
    else:
