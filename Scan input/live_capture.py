...
_process_name_cache = {}  # pid -> name (or None), cleared periodically to handle PID reuse


def resolve_process_name(pid):
    """Best-effort PID -> process name. Returns None if we don't have
    permission to see the process (common for processes owned by other
    users without elevated privileges) or it has already exited."""
    if not pid:
        return None
...
def capture_snapshot(local_ip, seen_devices, connections, conn_counter, device_counter):
    ...
    # PID cache is cleared every capture_snapshot call's caller loop via
    # clear_process_cache() below, so short-lived/reused PIDs don't get
    # attributed to the wrong process over a long-running session.
    ...
    for c in conns:
        ...
        process_name = resolve_process_name(getattr(c, "pid", None))
        ...
        for ip in (src_ip, dst_ip):
            if ip not in seen_devices:
                ...
                device_counter[0] += 1
                seen_devices[ip] = {
                    "device_id": f"D{device_counter[0]+1:03d}",
                    "ip": ip,
        ...
        connections.append({
            ...
            "process_name": process_name,  # None if unknown/no permission — handled everywhere downstream
        })
        count += 1
    return count
...
def clear_process_cache():
    """Called once per capture cycle (from the caller) so PID->name
    attribution doesn't go stale across a long-running session (PIDs
    get reused by the OS over time)."""
...
def run(duration_seconds, interval_seconds):
    ...
    while time.time() < t_end:
        n = capture_snapshot(local_ip, seen_devices, connections, conn_counter, device_counter)
        tick += 1
