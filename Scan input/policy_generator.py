...
def _ip_family(ip):
    """Return 4 or 6 for a valid source/destination IP address."""
...
def generate_iptables_rule(alert):
    ...
    family = _ip_family(src_ip)
    if dst_ip and _ip_family(dst_ip) != family:
        raise ValueError(f"Source/destination IP families do not match: {src_ip} -> {dst_ip}")
    firewall_cmd = "ip6tables" if family == 6 else "iptables"

    if dst_ip:
        # Specific source -> destination block
        rule = f"{firewall_cmd} -A FORWARD -s {src_ip} -d {dst_ip} -j DROP"
        ...
        rule = f"{firewall_cmd} -A FORWARD -s {src_ip} -m limit --limit 5/min -j ACCEPT\n" \
               f"{firewall_cmd} -A FORWARD -s {src_ip} -j DROP"
...
def generate_nftables_rule(alert):
    ...
    if dst_ip and _ip_family(dst_ip) != family:
        raise ValueError(f"Source/destination IP families do not match: {src_ip} -> {dst_ip}")
    ip_expr = "ip6" if family == 6 else "ip"

    if dst_ip:
        rule = f"nft add rule inet filter forward {ip_expr} saddr {src_ip} {ip_expr} daddr {dst_ip} drop"
    else:
        rule = f"nft add rule inet filter forward {ip_expr} saddr {src_ip} limit rate 5/minute accept"
